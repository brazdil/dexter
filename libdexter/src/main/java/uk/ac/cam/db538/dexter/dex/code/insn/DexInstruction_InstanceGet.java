...
public class DexInstruction_InstanceGet extends DexInstruction {
	...
	public static DexInstruction_InstanceGet parse(Instruction insn, CodeParserState parsingState) {
		final uk.ac.cam.db538.dexter.dex.code.insn.Opcode_GetPut opcode = Opcode_GetPut.convert_IGET(insn.opcode);
		if (insn instanceof Instruction22c && opcode != null) {
			...
			final org.jf.dexlib.Code.Format.Instruction22c insnInstanceGet = (Instruction22c)insn;
			final org.jf.dexlib.FieldIdItem refItem = (FieldIdItem)insnInstanceGet.getReferencedItem();
			DexRegister regTo;
			if (opcode == Opcode_GetPut.Wide) regTo = parsingState.getWideRegister(insnInstanceGet.getRegisterA()); else regTo = parsingState.getSingleRegister(insnInstanceGet.getRegisterA());
			final uk.ac.cam.db538.dexter.dex.code.reg.DexSingleRegister regObj = parsingState.getSingleRegister(insnInstanceGet.getRegisterB());
