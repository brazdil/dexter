...
public class ClassRenamer implements Serializable {
	...
	public ClassRenamer(DexFile file, HierarchyBuilder builder) {
		...
		for (final org.jf.dexlib.ClassDefItem clsItem : file.ClassDefsSection.getItems()) {
			final uk.ac.cam.db538.dexter.dex.type.DexTypeCache typeCache = builder.getTypeCache();
			final java.lang.String clsDesc = clsItem.getClassType().getTypeDescriptor();
			long i = 0;
			while (builder.hasClass(DexClassType.parse(addSuffix(clsDesc, i), typeCache))) i++;
			if (i != 0) addRule(clsDesc, addSuffix(clsDesc, i));
		}
	}
	/*
	...
	private void addRule(String original, String replacement) {
		if (!DexClassType.isClassDescriptor(original) || !DexClassType.isClassDescriptor(replacement)) throw new Error("Invalid class type descriptor"); else if (rules.containsKey(original)) throw new Error("Multiple name replacement rules for class " + original); else rules.put(original, replacement);
	}
	
	public String applyRules(String clazz) {
		final java.lang.String rule = rules.get(clazz);
		if (rule == null) return clazz; else return rule;
	}
}