...
public class DexInstruction_BinaryOpLiteral extends DexInstruction {
	private final DexSingleRegister regTarget;
	private final DexSingleRegister regSource;
	private final long literal;
	private final Opcode_BinaryOpLiteral insnOpcode;
	...
	public static DexInstruction_BinaryOpLiteral parse(Instruction insn, CodeParserState parsingState) {
		final uk.ac.cam.db538.dexter.dex.code.insn.Opcode_BinaryOpLiteral opcode = Opcode_BinaryOpLiteral.convert(insn.opcode);
		int regA;
		int regB;
		long lit;
		if (insn instanceof Instruction22s && opcode != null) {
			final org.jf.dexlib.Code.Format.Instruction22s insnBinaryOpLit16 = (Instruction22s)insn;
			...
			final org.jf.dexlib.Code.Format.Instruction22b insnBinaryOpLit8 = (Instruction22b)insn;
		...
		return new DexInstruction_BinaryOpLiteral(parsingState.getSingleRegister(regA), parsingState.getSingleRegister(regB), lit, opcode, parsingState.getHierarchy());
	}
	
	@Override
	public String toString() {
		return insnOpcode.name().toLowerCase() + "-int/lit " + regTarget.toString() + ", " + regSource.toString() + ", #" + literal;
	}
...
//                            });
	...
	public DexSingleRegister getRegTarget() {
		return this.regTarget;
	}
	
	@java.lang.SuppressWarnings("all")
	public DexSingleRegister getRegSource() {
	...
	@java.lang.SuppressWarnings("all")
	public Opcode_BinaryOpLiteral getInsnOpcode() {
