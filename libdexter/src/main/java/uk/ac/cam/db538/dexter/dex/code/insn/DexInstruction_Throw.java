...
public class DexInstruction_Throw extends DexInstruction {
	private final DexSingleRegister regFrom;
	...
	public static DexInstruction_Throw parse(Instruction insn, CodeParserState parsingState) {
		if (insn instanceof Instruction11x && insn.opcode == Opcode.THROW) {
			final org.jf.dexlib.Code.Format.Instruction11x insnThrow = (Instruction11x)insn;
			return new DexInstruction_Throw(parsingState.getSingleRegister(insnThrow.getRegisterA()), parsingState.getHierarchy());
	...
	public DexSingleRegister getRegFrom() {
		return this.regFrom;
	}
}