...
public class DexInstruction_StaticGet extends DexInstruction {
	...
	public static DexInstruction_StaticGet parse(Instruction insn, CodeParserState parsingState) {
		final uk.ac.cam.db538.dexter.dex.code.insn.Opcode_GetPut opcode = Opcode_GetPut.convert_SGET(insn.opcode);
		if (insn instanceof Instruction21c && opcode != null) {
			...
			final org.jf.dexlib.Code.Format.Instruction21c insnStaticGet = (Instruction21c)insn;
			final org.jf.dexlib.FieldIdItem refItem = (FieldIdItem)insnStaticGet.getReferencedItem();
			DexRegister regTo;
			if (opcode == Opcode_GetPut.Wide) regTo = parsingState.getWideRegister(insnStaticGet.getRegisterA()); else regTo = parsingState.getSingleRegister(insnStaticGet.getRegisterA());
