...
public class DexInstruction_MoveResult extends DexInstruction {
	...
	public static DexInstruction_MoveResult parse(Instruction insn, CodeParserState parsingState) {
		if (insn instanceof Instruction11x && (insn.opcode == Opcode.MOVE_RESULT || insn.opcode == Opcode.MOVE_RESULT_WIDE || insn.opcode == Opcode.MOVE_RESULT_OBJECT)) {
			final org.jf.dexlib.Code.Format.Instruction11x insnMoveResult = (Instruction11x)insn;
			final uk.ac.cam.db538.dexter.dex.code.reg.RegisterType opcode = RegisterType.fromOpcode(insn.opcode);
			if (opcode == RegisterType.WIDE_PRIMITIVE) return new DexInstruction_MoveResult(parsingState.getWideRegister(insnMoveResult.getRegisterA()), parsingState.getHierarchy()); else return new DexInstruction_MoveResult(parsingState.getSingleRegister(insnMoveResult.getRegisterA()), opcode == RegisterType.REFERENCE, parsingState.getHierarchy());
