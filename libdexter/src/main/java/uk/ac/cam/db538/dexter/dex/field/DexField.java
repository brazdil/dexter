...
public abstract class DexField {
	...
	private static List<DexAnnotation> init_ParseAnnotations(Dex dex, EncodedField fieldInfo, AnnotationDirectoryItem annoDir) {
		if (annoDir == null) return Collections.emptyList(); else return DexAnnotation.parseAll(annoDir.getFieldAnnotations(fieldInfo.field), dex.getTypeCache());
	}
	...
	public EncodedField writeToFile(DexFile outFile, DexAssemblingCache cache) {
		final uk.ac.cam.db538.dexter.hierarchy.FieldDefinition fieldDef = internal_GetFieldDef();
		final org.jf.dexlib.FieldIdItem fieldItem = cache.getField(fieldDef);
		final int accessFlags = DexUtils.assembleAccessFlags(fieldDef.getAccessFlags());
	...
	public static Cache<FieldDefinition, FieldIdItem> createAssemblingCache(final DexAssemblingCache cache, final DexFile outFile) {
		return new Cache<FieldDefinition, FieldIdItem>(){
			...
			protected FieldIdItem createNewEntry(FieldDefinition key) {
				return FieldIdItem.internFieldIdItem(outFile, cache.getType(key.getParentClass().getType()), cache.getType(key.getFieldId().getType()), cache.getStringConstant(key.getFieldId().getName()));
			}
		};
	}
	
	public FieldAnnotation assembleAnnotations(DexFile outFile, DexAssemblingCache cache) {
		if (_annotations.size() == 0) return null;
		final java.util.ArrayList<org.jf.dexlib.AnnotationItem> annoList = new ArrayList<AnnotationItem>(_annotations.size());
		for (final uk.ac.cam.db538.dexter.dex.DexAnnotation anno : _annotations) annoList.add(anno.writeToFile(outFile, cache));
		final org.jf.dexlib.AnnotationSetItem annoSet = AnnotationSetItem.internAnnotationSetItem(outFile, annoList);
		final org.jf.dexlib.AnnotationDirectoryItem.FieldAnnotation fieldAnno = new FieldAnnotation(cache.getField(internal_GetFieldDef()), annoSet);
		return fieldAnno;
	}
