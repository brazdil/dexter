...
public class DexInstruction_ArrayPut extends DexInstruction {
	...
	public static DexInstruction_ArrayPut parse(Instruction insn, CodeParserState parsingState) {
		final uk.ac.cam.db538.dexter.dex.code.insn.Opcode_GetPut opcode = Opcode_GetPut.convert_APUT(insn.opcode);
		if (insn instanceof Instruction23x && opcode != null) {
			...
			if (opcode == Opcode_GetPut.Wide) regFrom = parsingState.getWideRegister(insnArrayGet.getRegisterA()); else regFrom = parsingState.getSingleRegister(insnArrayGet.getRegisterA());
			return new DexInstruction_ArrayPut(regFrom, parsingState.getSingleRegister(insnArrayGet.getRegisterB()), parsingState.getSingleRegister(insnArrayGet.getRegisterC()), opcode, parsingState.getHierarchy());
...
//    }
	...
	protected DexClassType[] throwsExceptions() {
		if (opcode == Opcode_GetPut.Object) return this.hierarchy.getTypeCache().LIST_Error_Null_ArrayIndex_ArrayStore; else return this.hierarchy.getTypeCache().LIST_Error_Null_ArrayIndexOutOfBounds;
	}
