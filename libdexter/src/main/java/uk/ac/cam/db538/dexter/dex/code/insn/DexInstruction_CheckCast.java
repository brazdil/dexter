...
public class DexInstruction_CheckCast extends DexInstruction {
	...
	public static DexInstruction_CheckCast parse(Instruction insn, CodeParserState parsingState) {
		if (insn instanceof Instruction21c && insn.opcode == Opcode.CHECK_CAST) {
			...
			final org.jf.dexlib.Code.Format.Instruction21c insnCheckCast = (Instruction21c)insn;
			return new DexInstruction_CheckCast(parsingState.getSingleRegister(insnCheckCast.getRegisterA()), DexReferenceType.parse(((TypeIdItem)insnCheckCast.getReferencedItem()).getTypeDescriptor(), hierarchy.getTypeCache()), hierarchy);
