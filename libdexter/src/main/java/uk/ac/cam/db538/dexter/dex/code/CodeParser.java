...
public abstract class CodeParser {
	...
	public static DexCode parse(MethodDefinition methodDef, CodeItem codeItem, RuntimeHierarchy hierarchy) {
		final uk.ac.cam.db538.dexter.dex.code.CodeParserState parserCache = new CodeParserState(codeItem, hierarchy);
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.insn.DexInstruction> parsedInstructions = parseInstructions(codeItem, parserCache);
		final uk.ac.cam.db538.dexter.utils.Pair<uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexTryStart>, uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexTryEnd>> parsedTryBlocks = parseTryBlocks(codeItem.getTries(), parserCache);
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexTryStart> parsedTryStarts = parsedTryBlocks.getValA();
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexTryEnd> parsedTryEnds = parsedTryBlocks.getValB();
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexLabel> parsedLabels = parserCache.getListOfLabels();
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexCatch> parsedCatches = parserCache.getListOfCatches();
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexCatchAll> parsedCatchAlls = parserCache.getListOfCatchAlls();
		final uk.ac.cam.db538.dexter.dex.code.InstructionList instructionList = finalizeCode(parsedInstructions, parsedTryStarts, parsedTryEnds, parsedLabels, parsedCatches, parsedCatchAlls);
		final java.util.List<uk.ac.cam.db538.dexter.dex.code.DexCode.Parameter> params = parseParameters(methodDef, codeItem, parserCache);
	...
	private static List<Parameter> parseParameters(MethodDefinition methodDef, CodeItem codeItem, CodeParserState parserCache) {
		final uk.ac.cam.db538.dexter.dex.type.DexClassType definingClass = methodDef.getParentClass().getType();
		final boolean isStatic = methodDef.isStatic();
		final uk.ac.cam.db538.dexter.dex.type.DexPrototype prototype = methodDef.getMethodId().getPrototype();
		final int paramCount = prototype.getParameterCount(isStatic);
		final int paramRegCount = prototype.countParamWords(isStatic);
		final int regCount = codeItem.getRegisterCount();
		final java.util.ArrayList<uk.ac.cam.db538.dexter.dex.code.DexCode.Parameter> params = new ArrayList<Parameter>(paramCount);
		int regId = regCount - paramRegCount;
		for (int i = 0; i < paramCount; i++) {
			final uk.ac.cam.db538.dexter.dex.type.DexRegisterType paramType = prototype.getParameterType(i, isStatic, definingClass);
			final uk.ac.cam.db538.dexter.dex.code.reg.RegisterWidth paramWidth = paramType.getTypeWidth();
			DexStandardRegister paramReg;
			if (paramWidth == RegisterWidth.SINGLE) paramReg = parserCache.getSingleRegister(regId); else paramReg = parserCache.getWideRegister(regId);
	...
	private static FragmentList<DexInstruction> parseInstructions(CodeItem codeItem, CodeParserState parserCache) {
		final org.jf.dexlib.Code.Instruction[] dexlibInstructions = codeItem.getInstructions();
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.insn.DexInstruction> parsedCode = new FragmentList<DexInstruction>();
		long insnOffset = 0L;
		for (final org.jf.dexlib.Code.Instruction insn : dexlibInstructions) {
			final uk.ac.cam.db538.dexter.dex.code.insn.DexInstruction parsedInsn = parseInstruction(insn, parserCache);
			if (parsedInsn != null) parsedCode.add(new Fragment<DexInstruction>(insnOffset, parsedInsn));
	...
	private static Pair<FragmentList<DexTryStart>, FragmentList<DexTryEnd>> parseTryBlocks(TryItem[] tryItems, CodeParserState parserCache) {
		if (tryItems == null) return Pair.create(new FragmentList<DexTryStart>(), new FragmentList<DexTryEnd>());
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexTryStart> parsedTryStarts = new FragmentList<DexTryStart>();
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<uk.ac.cam.db538.dexter.dex.code.elem.DexTryEnd> parsedTryEnds = new FragmentList<DexTryEnd>();
		int counter = 1;
		for (final org.jf.dexlib.CodeItem.TryItem tryItem : tryItems) {
			// start/end offsets
			...
			final int catchallOffset = tryItem.encodedCatchHandler.getCatchAllHandlerAddress();
			DexCatchAll catchallHandler;
			if (catchallOffset < 0) catchallHandler = null; else catchallHandler = parserCache.getCatchAll(catchallOffset);
			// other Catch handlers
			final org.jf.dexlib.CodeItem.EncodedTypeAddrPair[] encodedHandlers = tryItem.encodedCatchHandler.handlers;
			final java.util.ArrayList<uk.ac.cam.db538.dexter.dex.code.elem.DexCatch> catchHandlers = new ArrayList<DexCatch>(encodedHandlers.length);
			for (final org.jf.dexlib.CodeItem.EncodedTypeAddrPair encodedHandler : encodedHandlers) {
				final int catchOffset = encodedHandler.getHandlerAddress();
				final uk.ac.cam.db538.dexter.dex.type.DexClassType catchType = DexClassType.parse(encodedHandler.exceptionType.getTypeDescriptor(), parserCache.getHierarchy().getTypeCache());
				catchHandlers.add(parserCache.getCatch(catchOffset, catchType));
			}
			// create TryStart and TryEnd
			final uk.ac.cam.db538.dexter.dex.code.elem.DexTryEnd tryEnd = new DexTryEnd(counter);
			final uk.ac.cam.db538.dexter.dex.code.elem.DexTryStart tryStart = new DexTryStart(counter, tryEnd, catchallHandler, catchHandlers);
			counter++;
	...
	private static InstructionList finalizeCode(FragmentList<DexInstruction> instructions, FragmentList<DexTryStart> tryStarts, FragmentList<DexTryEnd> tryEnds, FragmentList<DexLabel> labels, FragmentList<DexCatch> catches, FragmentList<DexCatchAll> catchAlls) {
		final java.util.ArrayList<uk.ac.cam.db538.dexter.dex.code.elem.DexCodeElement> finalInstructionList = new ArrayList<DexCodeElement>(instructions.size() + tryStarts.size() + tryEnds.size() + labels.size() + catches.size() + catchAlls.size());
		// Ordering is important! Reflects the desired order of combining the fragment lists
		final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<?>[] nonInstructionFragments = new FragmentList<?>[]{tryEnds, catches, catchAlls, labels, tryStarts};
		...
		while (!instructions.isEmpty()) {
			final uk.ac.cam.db538.dexter.dex.code.CodeParser.Fragment<uk.ac.cam.db538.dexter.dex.code.insn.DexInstruction> headInsn = instructions.pop();
			...
			for (final uk.ac.cam.db538.dexter.dex.code.CodeParser.FragmentList<?> fragList : nonInstructionFragments) {
				while (!fragList.isEmpty() && fragList.peek().getAbsoluteOffset() <= offset) {
					final uk.ac.cam.db538.dexter.dex.code.CodeParser.Fragment<? extends uk.ac.cam.db538.dexter.dex.code.elem.DexCodeElement> fragment = fragList.pop();
			...
			if (!instructions.isEmpty()) finalInstructionList.add(headInsn.getElement());
		}
	...
	static class Fragment<T extends DexCodeElement> extends Pair<Long, T> implements Comparable<Fragment<T>> {
		...
		public String toString() {
			if (getValB() == null) return getValA().toString() + ": null"; else return getValA().toString() + ": " + getValB().toString();
		}
	}
	...
	static class FragmentList<T extends DexCodeElement> extends LinkedList<Fragment<T>> {
		...
		public void dump() {
			for (final uk.ac.cam.db538.dexter.dex.code.CodeParser.Fragment<T> fragment : this) System.err.println(fragment.toString());
		}
	}
}