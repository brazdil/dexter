...
public class DexInstruction_Return extends DexInstruction {
	...
	private final RegisterType opcode;
	...
	public static DexInstruction_Return parse(Instruction insn, CodeParserState parsingState) {
		if (insn instanceof Instruction11x && (insn.opcode == Opcode.RETURN || insn.opcode == Opcode.RETURN_WIDE || insn.opcode == Opcode.RETURN_OBJECT)) {
			final org.jf.dexlib.Code.Format.Instruction11x insnReturn = (Instruction11x)insn;
			final uk.ac.cam.db538.dexter.dex.code.reg.RegisterType opcode = RegisterType.fromOpcode(insn.opcode);
			if (opcode == RegisterType.WIDE_PRIMITIVE) return new DexInstruction_Return(parsingState.getWideRegister(insnReturn.getRegisterA()), parsingState.getHierarchy()); else return new DexInstruction_Return(parsingState.getSingleRegister(insnReturn.getRegisterA()), opcode == RegisterType.REFERENCE, parsingState.getHierarchy());
