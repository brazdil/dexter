	...
	public static Cache<DexType, TypeIdItem> createAssemblingCache(final DexAssemblingCache cache, final DexFile outFile) {
		return new Cache<DexType, TypeIdItem>(){
			...
			protected TypeIdItem createNewEntry(DexType type) {
				return TypeIdItem.internTypeIdItem(outFile, cache.getStringConstant(type.getDescriptor()));
			}
		};
	}
	...
	public static Cache<List<? extends DexRegisterType>, TypeListItem> createAssemblingCacheForLists(final DexAssemblingCache cache, final DexFile outFile) {
		return new Cache<List<? extends DexRegisterType>, TypeListItem>(){
			...
			protected TypeListItem createNewEntry(List<? extends DexRegisterType> typeList) {
				final java.util.ArrayList<org.jf.dexlib.TypeIdItem> dexTypeList = new ArrayList<TypeIdItem>(typeList.size());
				for (final uk.ac.cam.db538.dexter.dex.type.DexRegisterType type : typeList) dexTypeList.add(cache.getType(type));
