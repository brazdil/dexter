...
public class DexInstruction_StaticPut extends DexInstruction {
	...
	public static DexInstruction_StaticPut parse(Instruction insn, CodeParserState parsingState) {
		final uk.ac.cam.db538.dexter.dex.code.insn.Opcode_GetPut opcode = Opcode_GetPut.convert_SPUT(insn.opcode);
		if (insn instanceof Instruction21c && opcode != null) {
			...
			final org.jf.dexlib.Code.Format.Instruction21c insnStaticPut = (Instruction21c)insn;
			final org.jf.dexlib.FieldIdItem refItem = (FieldIdItem)insnStaticPut.getReferencedItem();
			DexRegister regFrom;
			if (opcode == Opcode_GetPut.Wide) regFrom = parsingState.getWideRegister(insnStaticPut.getRegisterA()); else regFrom = parsingState.getSingleRegister(insnStaticPut.getRegisterA());
			StaticFieldDefinition fieldDef = hierarchy.getBaseClassDefinition(DexClassType.parse(refItem.getContainingClass().getTypeDescriptor(), hierarchy.getTypeCache())).getAccessedStaticField(DexFieldId.parseFieldId(refItem.getFieldName().getStringValue(), DexRegisterType.parse(refItem.getFieldType().getTypeDescriptor(), hierarchy.getTypeCache()), hierarchy.getTypeCache()));
