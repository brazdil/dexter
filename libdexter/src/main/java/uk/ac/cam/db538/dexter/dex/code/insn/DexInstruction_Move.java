...
public class DexInstruction_Move extends DexInstruction {
	...
	public static DexInstruction_Move parse(Instruction insn, CodeParserState parsingState) {
		int regA;
		int regB;
		if (insn instanceof Instruction12x && (insn.opcode == Opcode.MOVE || insn.opcode == Opcode.MOVE_WIDE || insn.opcode == Opcode.MOVE_OBJECT)) {
			final org.jf.dexlib.Code.Format.Instruction12x insnMove = (Instruction12x)insn;
			regA = insnMove.getRegisterA();
			regB = insnMove.getRegisterB();
		} else if (insn instanceof Instruction22x && (insn.opcode == Opcode.MOVE_FROM16 || insn.opcode == Opcode.MOVE_WIDE_FROM16 || insn.opcode == Opcode.MOVE_OBJECT_FROM16)) {
			final org.jf.dexlib.Code.Format.Instruction22x insnMoveFrom16 = (Instruction22x)insn;
			regA = insnMoveFrom16.getRegisterA();
			regB = insnMoveFrom16.getRegisterB();
		} else if (insn instanceof Instruction32x && (insn.opcode == Opcode.MOVE_16 || insn.opcode == Opcode.MOVE_WIDE_16 || insn.opcode == Opcode.MOVE_OBJECT_16)) {
			final org.jf.dexlib.Code.Format.Instruction32x insnMove16 = (Instruction32x)insn;
			regA = insnMove16.getRegisterA();
			regB = insnMove16.getRegisterB();
		} else throw FORMAT_EXCEPTION;
		final uk.ac.cam.db538.dexter.dex.code.reg.RegisterType opcode = RegisterType.fromOpcode(insn.opcode);
		if (opcode == RegisterType.WIDE_PRIMITIVE) {
			return new DexInstruction_Move(parsingState.getWideRegister(regA), parsingState.getWideRegister(regB), parsingState.getHierarchy());
		} else {
			return new DexInstruction_Move(parsingState.getSingleRegister(regA), parsingState.getSingleRegister(regB), opcode == RegisterType.REFERENCE, parsingState.getHierarchy());
		}
	}
