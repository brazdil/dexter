...
public enum Opcode_GetPut {
	...
	Short("-short"),
	Wide("-wide");
	private final String AsmSuffix;
	...
	public static void checkTypeAgainstOpcode(DexRegisterType type, Opcode_GetPut opcode) {
		...
		if (!typeOK) throw new Error("Source/target type doesn\'t match the instruction\'s opcode");
	}
	
	public static Opcode_GetPut getOpcodeFromType(DexRegisterType type) {
		if (type instanceof DexReferenceType) return Object; else if (type instanceof DexInteger || type instanceof DexFloat) return IntFloat; else if (type instanceof DexBoolean) return Boolean; else if (type instanceof DexByte) return Byte; else if (type instanceof DexChar) return Char; else if (type instanceof DexShort) return Short; else if (type instanceof DexLong || type instanceof DexDouble) return Wide; else throw new Error("Type given to instruction is not supported");
	}
	
	@java.lang.SuppressWarnings("all")
	public String getAsmSuffix() {
		return this.AsmSuffix;
	}
}